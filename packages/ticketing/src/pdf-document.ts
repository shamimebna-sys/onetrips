...
export type PdfFonts = { regular: PDFFont; bold: PDFFont; unicode: PDFFont; unicodeBold: PDFFont };

const fontFamily = new WeakMap<PDFFont, PdfFonts>();

function rememberFonts(fonts: PdfFonts) {
  fontFamily.set(fonts.regular, fonts);
  fontFamily.set(fonts.bold, fonts);
  fontFamily.set(fonts.unicode, fonts);
  fontFamily.set(fonts.unicodeBold, fonts);
}
...
export function pickFont(font: PDFFont, value: string): PDFFont {
  if (canEncode(font, value)) return font;
  const family = fontFamily.get(font);
  if (!family) return font;
  const unicode = font === family.bold || font === family.unicodeBold ? family.unicodeBold : family.unicode;
  if (canEncode(unicode, value)) return unicode;
  return font;
}
...
export type PdfFlow = {
  ...
  measureJourney(segment: JourneySegment): number;
  measureConnection(airport?: string | null): number;
  ...
  save(): Promise<Buffer>;
  saveWithoutFooter(): Promise<Buffer>;
};
...
export function displayMoney(value: string | null | undefined) {
  ...
  if (text.startsWith("৳")) {
    const amount = text.replace(/^৳\s*/, "").trim();
    return amount ? `BDT ${amount}` : null;
  }
  return text;
}

export function textWidth(font: PDFFont, value: string, size: number) {
  const used = pickFont(font, value);
  return used.widthOfTextAtSize(value, size);
}
...
export function fitMoney(value: string, font: PDFFont, maxWidth: number) {
  ...
  const used = pickFont(font, text);
  for (const size of [11, 10, 9, 8, 7]) {
    if (used.widthOfTextAtSize(text, size) <= maxWidth) return { text, size, font: used };
  }
  return { text: wrapText(text, used, 7, maxWidth)[0] ?? text, size: 7, font: used };
}
...
function paint(
  ...
  page.drawText(safe, { x, y, size, font: used, color });
  return width;
}
...
function paintRight(
  ...
  if (!safe) return;
  const used = pickFont(font, safe);
  paint(page, safe, right - used.widthOfTextAtSize(safe, size), y, size, used, color, region);
}
...
function startNewPage(layout: Layout) {
  if (layout.y >= contentTop(layout.kind) - 0.5) return;
  addPage(layout);
}
...
function drawInlineFacts(layout: Layout, fields: MetaField[]) {
  ...
  const height = 8 + lines.length * 16;
  ...
  for (const row of lines) {
    ...
    row.forEach((part, index) => {
      ...
      valueLines.forEach((line, lineIndex) => {
        paint(layout.page, line, x, layout.y - lineIndex * 11, 8.5, layout.fonts.bold, NAVY);
      });
...
function measureJourney(fonts: PdfFonts, segment: JourneySegment) {
  ...
  const aircraft = present(segment.aircraft);
  const metaCount = [cabin, duration, aircraft].filter(Boolean).length;
  const meta = metaCount ? Math.ceil(metaCount / 2) * 16 : 0;
  const bags = baggageRows(segment);
  const bagHeight = bags.length ? 16 + Math.ceil(bags.length / 2) * 16 : 0;
  return 28 + airlineLines * 13 + 12 + Math.max(originH, destH, 62) + meta + bagHeight + 18;
}
...
function drawJourney(layout: Layout, segment: JourneySegment, index: number) {
  ...
  if (aircraft) meta.push({ label: "Aircraft", value: aircraft });
...
function connectionHeight(fonts: PdfFonts, airport?: string | null) {
  const code = present(airport)?.toUpperCase() ?? null;
  const shown = code && textWidth(fonts.bold, code, 9) > 80 ? wrapText(code, fonts.bold, 9, 80)[0] ?? code : code;
  return { height: shown ? 46 : 36, shown };
}

function drawConnection(layout: Layout, airport?: string | null) {
  const { height, shown } = connectionHeight(layout.fonts, airport);
...
function drawNote(layout: Layout, title: string, body: string) {
  const lines = wrapText(body, layout.fonts.regular, 9.5, CONTENT_WIDTH);
  ensureSpace(layout, 28 + Math.min(lines.length, 4) * 14);
  layout.page.drawLine({
    start: { x: MARGIN, y: layout.y + 4 },
    end: { x: PAGE_WIDTH - MARGIN, y: layout.y + 4 },
    thickness: 0.45,
  ...
  paint(layout.page, title, MARGIN, layout.y - 8, 9, layout.fonts.bold, NAVY);
  advance(layout, 22);
  for (const line of lines) {
    ensureSpace(layout, 14);
    paint(layout.page, line, MARGIN, layout.y, 9.5, layout.fonts.regular, INK);
    advance(layout, 14);
  }
  advance(layout, 12);
}
...
function drawTotals(layout: Layout, rows: Array<{ label: string; value: string; emphasize?: boolean }>) {
  ...
  for (const row of rows) {
    if (row.emphasize) {
      ...
      paintRight(layout.page, display.text, PAGE_WIDTH - MARGIN - 10, layout.y - 2, Math.max(display.size, 11), display.font, WHITE);
      advance(layout, 22);
    } else {
      ensureSpace(layout, 16);
      paint(layout.page, row.label, left + 10, layout.y, 8.5, layout.fonts.regular, MUTED);
      const display = fitMoney(row.value, layout.fonts.bold, width - 110);
      paintRight(layout.page, display.text, PAGE_WIDTH - MARGIN - 10, layout.y, display.size, display.font, NAVY);
      advance(layout, 16);
    }
  }
}
...
export async function openDocument(options: {
  ...
  const fonts: PdfFonts = {
    ...
    unicode: unicodeFonts.unicode,
    unicodeBold: unicodeFonts.unicodeBold,
  };
  rememberFonts(fonts);
  doc.setTitle(pdfMeta(options.docTitle));
  doc.setAuthor("ONETRIPS");
  doc.setSubject(pdfMeta(options.subject ?? options.title));
  doc.setCreator("ONETRIPS");
  doc.setProducer("ONETRIPS");
  doc.setKeywords((options.keywords ?? []).map((value) => pdfMeta(value)).filter(Boolean));
  ...
  return {
    ...
    newPage: () => startNewPage(layout),
    chapter: () => addPage(layout),
    ...
    measureJourney: (segment) => measureJourney(layout.fonts, segment),
    measureConnection: (airport) => connectionHeight(layout.fonts, airport).height,
    ...
    saveWithoutFooter: async () => Buffer.from(await doc.save()),
  };
}

export async function concatTicketChapters(
  chapters: Buffer[],
  footerParts: string[],
  info: { title: string; subject: string; keywords: string[] },
): Promise<Buffer> {
  if (chapters.length === 0) throw new Error("Ticket PDF has no chapters");
  const doc = await PDFDocument.create();
  const unicodeFonts = await embedUnicodeFonts(doc);
  const fonts: PdfFonts = {
    regular: await doc.embedFont(StandardFonts.Helvetica),
  ...
  rememberFonts(fonts);
  doc.setTitle(pdfMeta(info.title));
  doc.setAuthor("ONETRIPS");
  doc.setSubject(pdfMeta(info.subject));
  doc.setCreator("ONETRIPS");
  doc.setProducer("ONETRIPS");
  doc.setKeywords(info.keywords.map((value) => pdfMeta(value)).filter(Boolean));

  for (const bytes of chapters) {
  ...
  pages.forEach((page, index) => {
    drawFooter(page, fonts, footerParts, index + 1, pages.length);
  });
  return Buffer.from(await doc.save());
}
